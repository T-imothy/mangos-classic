/*
  CMaNGOS Classic -> TBC single-character copy
  ------------------------------------------------------------
  Target: current stock CMaNGOS mangos-classic and mangos-tbc
  Database: MySQL 5.7+/8.0 or MariaDB 10.2+

  IMPORTANT
  - Back up both character databases before using this file.
  - Stop the destination TBC world server while executing the copy.
...
SET @source_character_db       = 'characters_classic';
SET @source_auth_db            = 'realmd_classic';
SET @destination_character_db  = 'characters_tbc';
SET @destination_auth_db       = 'realmd_tbc';
SET @destination_world_db      = 'mangos_tbc';
...
DROP PROCEDURE IF EXISTS mantech_tmp_classic_to_tbc;
...
CREATE PROCEDURE mantech_tmp_classic_to_tbc()
main: BEGIN
    ...
    IF v_missing_required > 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'TBC characters has required columns unavailable from the Classic schema.';
    END IF;
    ...
    -- Spells existing in TBC. Talent data is deliberately not translated;
    -- at_login includes RESET_TALENTS so TBC rebuilds a valid talent state.
    CALL mantech_tmp_copy_guid_table(
        @source_character_db, @destination_character_db,
...
CALL mantech_tmp_classic_to_tbc();
