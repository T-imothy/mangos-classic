/*
  CMaNGOS Classic -> WotLK single-character copy
  ------------------------------------------------------------
  Target: current stock CMaNGOS mangos-classic and mangos-wotlk
...
DROP PROCEDURE IF EXISTS mantech_tmp_classic_to_wotlk;
...
CREATE PROCEDURE mantech_tmp_classic_to_wotlk()
main: BEGIN
    ...
    IF v_missing_required > 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'WotLK characters has required columns unavailable from the Classic schema.';
    END IF;
...
CALL mantech_tmp_classic_to_wotlk();
