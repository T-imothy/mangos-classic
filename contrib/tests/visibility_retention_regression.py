...
struct Camera; struct Player; struct Map; struct WorldObject; struct Creature; struct Unit;
using WorldObjectSet=std::set<WorldObject*>;
struct WorldPacket; struct Session { int sends=0; void SendPacket(WorldPacket*) { ++sends; } void SendPacket(WorldPacket const&) { ++sends; } };
struct WorldPacket {};
struct UpdateData {
    GuidSet removed;
    ...
    size_t GetPacketCount() const { return HasData()?1:0; }
    WorldPacket BuildPacket(size_t) { return {}; }
    GuidSet const& GetOutOfRangeGUIDs() const { return removed; }
};
struct Visibility {
...
struct Player:Unit {
    ...
    Camera& GetCamera();
    void UpdateVisibilityOf(WorldObject*,WorldObject* obj) { if (!obj->visible) RemoveAtClient(obj); }
...
struct Camera { Player* owner; Player* GetOwner() { return owner; } WorldObject* GetBody() { return owner; } };
Camera& Player::GetCamera() { static Camera c{this}; c.owner=this; return c; }
struct ObjectAccessor { static Player* FindPlayer(ObjectGuid) { return nullptr; } };
...
void exceptions(bool real) {
    ...
    for(auto* obj:{&passenger,&farVisible,&farHidden}) {
...
def main():
    ...
    if args.baseline:
        notify_src = subprocess.check_output(['git','show','1a1f178fffdccea9e39df644221bd78ffb466408:'+notify_path],cwd=root,text=True)
